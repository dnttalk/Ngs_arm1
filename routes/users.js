...
router.post('/login', function (req, res) {
    ...
    fs.readFile(path.join(__dirname, "../data/data.json"), "utf-8", function (error, data) {
        if (error) {
            ...
            for (let obj of arr) {
                if (obj.user != user && obj.pwd != pwd) {
                    res.send(`<h1 style='color:red'>! Login fail</h1> userName&Password error<script>setTimeout(function back(){window.location='/login.html'},3000)</script><p>Login failed! After 3 seconds, it will automatically return to the login interface.....</p>`);
                } else if (obj.user != user) {
                    res.send(`<h1 style='color:red'>! Login fail</h1> userName error<script>setTimeout(function(){window.location='/login.html'},3000)</script><p>Login failed! After 3 seconds, it will automatically return to the login interface.....</p>`);
                } else {
                    res.send(`<h1 style='color:red'>! Login fail</h1> Password error<script>setTimeout(function(){window.location='/login.html'},3000)</script><p>Login failed! After 3 seconds, it will automatically return to the login interface.....</p>`);
                }
                return;
            }
        }
